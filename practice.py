...
def available_moves(piece_name,current_position,piece_dict):
    ...
    if 'pawn' in piece_name:
        ...
        for i in range(1,9):
            horizontal.append(pygame.Vector2(i,current_position.y))
            vertical.append(pygame.Vector2(current_position.x,i))


        for x in piece_dict:
            if current_position.x >= x['position'].x and current_position.y == x['position'].y:
                for y in horizontal:    
                    horizontal.remove(y)
                
            

        legal_squares += horizontal
        legal_squares += vertical

        for x in piece_dict:
            if x['position'] in legal_squares:
